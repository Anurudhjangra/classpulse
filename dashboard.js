(function () {
  ...
  async function checkStudent() {
    ...
    if (!currentGroup) { toast("Select a class first", "bad"); return; }
    if (!roll) { toast("Enter a roll number", "bad"); return; }
    try {
      ...
      if (!r || (!r.present && !r.absent)) {
        box.innerHTML = '<div class="empty" style="padding:12px"><div class="ico">🔎</div>No record found for Roll ' + roll + ' yet (student may not exist or attendance not marked).</div>';
        return;
      }
  ...
  async function loadLowAttendance() {
    ...
    if (!currentGroup) {
      box.innerHTML = '<div class="empty" style="padding:12px"><div class="ico">🏫</div>Select a class from the class bar above</div>';
      return;
    }
    try {
      ...
      if (!withAtt.length) {
        box.innerHTML = '<div class="empty" style="padding:12px"><div class="ico">📭</div>No attendance records yet</div>';
        return;
      }
  ...
  initClassBar((g) => {
    currentGroup = g;
    document.getElementById("sc-class").textContent = g ? `${g.className} · Section ${g.section}` : "Select a class";
    loadLowAttendance();
  });
  load();
})();
